...
static void countApplesAndOranges(int s, int t, int a, int b, int apples_count, int* apples, int oranges_count, int* oranges) {
	int	ans_apple = 0, ans_orange = 0;

	for (int i = 0; i < apples_count; i++) {
		int	pos_apple = a + apples[i];
		ans_apple += (pos_apple >= s && pos_apple <= t) ? 1 : 0;
	}
	for (int i = 0; i < oranges_count; i++) {
		int	pos_orange = b + oranges[i];
		ans_orange += (pos_orange >= s && pos_orange <= t) ? 1 : 0;
	}
	printf("%d\n%d\n", ans_apple, ans_orange);
}
